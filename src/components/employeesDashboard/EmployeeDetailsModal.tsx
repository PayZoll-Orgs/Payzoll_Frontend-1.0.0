  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Header */}
          <div className="p-6 border-b border-[#3B4058]/30 flex items-center justify-between">
            <h2 className="text-xl font-bold text-[#c8ceee] font-mono text-shadow-glow">Employee Details</h2>
            <button 
              onClick={onClose} 
          ...
          <div className="p-6 space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-lg font-semibold text-[#c8ceee] font-mono">{employee.name}</p>
              </div>
              <div className="flex items-center gap-2">
                <Briefcase className="w-5 h-5 text-[#93c5fd]" />
                <div>
                  <label className="text-sm text-gray-400 font-mono">Designation</label>
                  <p className="text-lg font-semibold text-[#c8ceee] font-mono">{employee.designation}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-lg font-semibold text-[#c8ceee] font-mono">{employee.walletAddress}</p>
                </div>
              </div>
              <div>
                <label className="text-sm text-gray-400 font-mono">Monthly Salary (USD)</label>
                <p className="text-lg font-semibold text-[#c8ceee] font-mono">${employee.salary}</p>
              </div>
            </div>

            {/* Payment History Section */}
            <div className="pt-4 border-t border-[#3B4058]/30">
              <h3 className="text-lg font-semibold mb-4 flex items-center text-[#c8ceee] font-mono text-shadow-glow">
                <Calendar className="w-5 h-5 text-[#93c5fd] mr-2" />
                Payment History
              </h3>
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto scrollbar-hide">
                {paymentHistory.map((payment) => (
                  <div 
                    key={payment.id}
                    className="bg-[#1D202D] rounded-xl p-4 border border-[#3B4058]/30 hover:border-[#93c5fd]/30 transition-all"
                  >
                    <div className="flex justify-between items-center">
                      <div className="flex items-center">
                        <StatusIcon status={payment.status} />
                        <span className="ml-2 font-medium text-[#c8ceee] font-mono">${payment.amount}</span>
                      </div>
          ...
          {/* Close Button */}
          <div className="p-6 border-t border-[#3B4058]/30 flex justify-end">
            <button
              onClick={onClose}
              className="px-6 py-2 rounded-xl bg-[#1D202D] hover:bg-[#252837] text-white transition-colors border border-[#3B4058]/30 font-mono"
            >
              Close
            </button>
          </div>
