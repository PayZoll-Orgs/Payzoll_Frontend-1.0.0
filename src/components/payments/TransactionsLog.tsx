...
const StatusIcon: React.FC<StatusIconProps> = ({ status }) => {
  ...
  if (status === "pending") return <Clock className="w-3 h-3 sm:w-4 sm:h-4 text-[#B38D36]" />;
...
const TransactionsLog = () => {
  ...
  return (
    <div className="w-full bg-[#131620]/90 backdrop-blur-sm border border-[#22304a]/30 rounded-xl p-3 sm:p-4 md:p-5 lg:p-6 
      ...
      <div className="flex flex-col xs:flex-row justify-between items-start xs:items-center mb-3 sm:mb-4 md:mb-5 lg:mb-6 gap-2 xs:gap-0">
        ...
        <button 
          className="bg-[#0c0f16]/80 backdrop-blur-sm text-white px-2 sm:px-3 md:px-4 py-1 sm:py-2 rounded-xl flex items-center space-x-1 sm:space-x-2 shadow-md
                   hover:bg-[#0c0f16] transition-all border border-[#22304a]/30 text-xs sm:text-sm"
          aria-label="Export transactions"
        >
          <Download className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 text-[#2D8B75]" />
      ...
      <div className="relative mb-2 sm:mb-3 md:mb-4">
        <input
          ...
          className="w-full bg-[#0c0f16]/80 backdrop-blur-sm border border-[#22304a]/30 text-white rounded-xl py-1 sm:py-2 pl-8 sm:pl-10 pr-3 sm:pr-4
        ...
        {filteredTransactions.length > 0 ? (
          filteredTransactions.map((tx, index) => (
            <motion.div
              ...
              className={`flex flex-col sm:flex-row justify-between items-start sm:items-center p-2 sm:p-3 md:p-4 rounded-xl border border-[#22304a]/30 
                          bg-[#0c0f16]/80 backdrop-blur-sm hover:border-[#2D8B75]/30 transition-all`}
            >
      ...
      {/* Footer */}
      <div className="mt-3 sm:mt-4 pt-2 sm:pt-3 md:pt-4 border-t border-[#22304a]/30 flex flex-col xs:flex-row items-start xs:items-center justify-between text-xs sm:text-sm gap-2 xs:gap-0">
        <span className="text-gray-400 font-mono">
          Showing recent {filteredTransactions.length} transaction
          {filteredTransactions.length !== 1 ? "s" : ""}
        </span>
        <button className="text-[#2D8B75] hover:text-[#2D8B75]/80 transition-colors font-mono">
